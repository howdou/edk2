...
VERSION = "4.2.0-dev-edk2-20190318"
...
class CacheLock:
    ...
    @staticmethod
    def forPath(path):
        timeoutMs = int(os.environ.get('CLCACHE_OBJECT_CACHE_TIMEOUT_MS', 30 * 60 * 1000))
...
class CacheFileStrategy:
    ...
    def clean(self, stats, maximumSize):
        ...
        # Free at least 50% to avoid cleaning up too often which
        # is a big performance hit with large caches.
        effectiveMaximumSizeOverall = maximumSize * 0.5
...
class InvalidMakeCacheInputSrcFiles(AnalysisError):
    pass

class InvalidMakeCacheInputLibs(AnalysisError):
    pass

class InvalidMakeCacheOutputFiles(AnalysisError):
    pass
...
def CheckMakeCache(Command, WorkingDir):
    cache = Cache()
    return True;


def MakeCacheIoFilesAnalyze(cmdline: List[str]) -> Tuple[List[Tuple[str, str]], List[str]]:
    options, inputFiles = CommandLineAnalyzer.parseArgumentsAndInputFiles(cmdline)
    # Use an override pattern to shadow input files that have
    # already been specified in the function above
    inputFiles = {inputFile: '' for inputFile in inputFiles}
...
def SaveMakeCache(Command, WorkingDir):
    cache = Cache()
    try:
        InputSrcFiles, InputLibs, OutputFiles = MakeCacheIoFilesAnalyze(WorkingDir)
    except InvalidMakeCacheInputSrcFiles:
        printTraceStatement("Invalid MakeCache InputSrcFiles in module:{}".format(WorkingDir))
        return InvalidMakeCacheInputSrcFiles
    except InvalidMakeCacheInputLibs:
        printTraceStatement("Invalid MakeCache InputLibs in module:{}".format(WorkingDir))
        return InvalidMakeCacheInputLibs
    except InvalidMakeCacheOutputFiles:
        printTraceStatement("Invalid MakeCache OutputFiles in module:{}".format(WorkingDir))
        return InvalidMakeCacheOutputFiles
    return True;
